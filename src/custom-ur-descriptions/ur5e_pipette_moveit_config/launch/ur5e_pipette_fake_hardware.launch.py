...
def generate_launch_description():
    """
    Launch file for UR5e with pipette using fake hardware (simulation).
    This launch file sets up the complete MoveIt stack with simulated hardware.
    """
    ...
    # Force fake hardware mode - use use_fake_hardware parameter for XACRO
    xacro_args = {
        "name": "ur", 
        "ur_type": LaunchConfiguration("ur_type"), 
        "tf_prefix": "", 
        "use_fake_hardware": "true"  # Always use fake hardware
    }

    # UR Control with fake hardware - use use_fake_hardware for ur_control launch
    ur_control_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            "ur_type": LaunchConfiguration("ur_type"),
            "robot_ip": "192.168.1.101",  # Not used in fake mode but required
            "launch_rviz": "false",
            "description_package": LaunchConfiguration("description_package"),
            "description_file": LaunchConfiguration("description_file"),
            "controllers_file": LaunchConfiguration("controllers_file"),
            "use_fake_hardware": "true",  # Force fake hardware
        }.items()
    )
